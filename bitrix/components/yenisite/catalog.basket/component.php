...
if(CModule::IncludeModule("yenisite.market"))
{
	...
	if($action == "payment")
	{
		$order_id = htmlspecialchars($_REQUEST["id"]);
			...
			switch($type)
			{
				...
				case "S":
					...
					if($arProperty[$code]["USER_TYPE"] == "HTML")
					{
						...
						$arResult["DISPLAY_PROPERTIES"][$code]["DEFAULT_VALUE"] = $arProperty[$code]["DEFAULT_VALUE"];
					}
					else
					{
				...
				case "E":
					...
					{
						$arres = $ob->GetFields();
						$resProp = CIBlockElement::GetList(array(), array('ID'=>$arres['ID']), false, array(), array("ID", "NAME", "PROPERTY_PRICE", "PREVIEW_TEXT", "PROPERTY_PATH_TO_ACTION"));
...
uasort($arResult["DISPLAY_PROPERTIES"], 'sortprop');
	...
	if(is_array($_SESSION["YEN_MARKET_BASKET"]))
	{
		foreach($_SESSION["YEN_MARKET_BASKET"] as $key=>$value)
		{
			...
			if(isset($_REQUEST["SIZE"][$key]) && $_REQUEST["SIZE"][$key]!=$res["PROPERTIES"]["SIZE"])
			{
				unset($res["PROPERTIES"]["SIZE"]);
				$res["PROPERTIES"]["SIZE"]=$_REQUEST["SIZE"][$key];
			}
...
//r($res["PROPERTIES"]);
			...
			/*if(isset($_REQUEST["SIZE"][$key]) && $_REQUEST["SIZE"][$key]!=$res["PROPERTIES"]["SIZE"]["VALUE"])
			{
				$res["PROPERTIES"]["SIZE"]["VALUE"]=$_REQUEST["SIZE"][$key];
			}*/
