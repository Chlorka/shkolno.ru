...
<?endif;?>
	<div class="yen-bs-count yen-bs-up">
		<div class="yen-bs-node">
			...
			<strong id="YS_BS_COUNT_PRODUCT"><?=$arResult['NUM_PRODUCTS'];?></strong><span id="YS_BS_COUNT_STRING"><?=yenisite_declOfNum($arResult['NUM_PRODUCTS'], array(GetMessage('YS_BS_PRODUCT1'), GetMessage('YS_BS_PRODUCT2'), GetMessage('YS_BS_PRODUCT0')));?></span><br><?=GetMessage('YS_BS_IN_SUMM');?><strong id="YS_BS_TOTALSUM_TOP"><?=$arResult['allSum_FORMAT'];?></strong> <b class="yen-bs-rubl"><?=$arResult['CURRENCY'];?></b><img class="yen-bs-icon" src="<?=$arParams['BASKET_ICON'];?>" title="<?=GetMessage('YS_BS_IN_BASKET');?>" alt="<?=GetMessage('YS_BS_BASKET');?>"/>
		</div>
		<div class="yen-bs-popup yen-bs-closed" id="yen-bs-bag-popup">
			<div class="yen-bs-rasp"></div>
			<a class="yen-bs-close" onclick="yenisite_bs_close()" title="<?=GetMessage('YS_BS_CLOSE');?>">'</a>
			<table>
				<tbody>
					<tr>
							...
							<?foreach ($arResult['ITEMS'] as $i=>$arItem)
							{
								...
								<tr id="YS_BS_ROW_<?=$i;?>">
									...
									<td class="yen-bs-t_count">
										...
										<button onclick="yen_setQuantity('<?=$i;?>', 'p'); return false;" class="yen-bs-button4">+</button> <button onclick="yen_setQuantity('<?=$i;?>', 'm'); return false;" class="yen-bs-button5">-</button>
										<?else:?>
									...
									<td class="yen-bs-t_delete">
										<button onclick="yen_setQuantity('<?=$i;?>', 'd'); return false;" class="yen-ys-button6 yen-bs-sym">'</button>
									</td>
								</tr>
								<?
								}
