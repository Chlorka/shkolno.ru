...
<section id="ww-section_filter">
	<?$url_section=$arResult["FOLDER"].$arResult["VARIABLES"]["SECTION_CODE"].'/';?>
	<a class="<?if($_REQUEST["list"]=='show_all'):?>ww-selection<?endif;?>"  href="<?=$url_section?>?list=show_all">Все модели</a>
	<?foreach($arResult["LIST_SECTION_SECOND"] as $index=>$value):
		$arrList[]=$value['CODE'];?>
		<a class="<?if($_REQUEST["list"]==$value['CODE']):?>ww-selection<?endif;?>" href="<?=$url_section?>?list=<?=$value['CODE']?>"><?=$value['NAME']?></a>
	<?endforeach;?>
</section>
<?endif;?>
...
if(CModule::IncludeModule("iblock"))
 {
	...
	$db_props = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $ElementID, array("sort" => "asc"), Array("CODE"=>"TYPE"));
	if($ar_props = $db_props->Fetch())
	{	if($ar_props["VALUE"]>0)
	    $GLOBALS['arrFilter_form']['PROPERTY_TYPE'] = $ar_props["VALUE"];
	}
	$GLOBALS['arrFilter_form']['SECTION_ID'] = $arResult["TREE_SECTIONS"][0]["ID"];
	$GLOBALS['arrFilter_form']['INCLUDE_SUBSECTIONS'] = 'Y';
	//r($arResult["TREE_SECTIONS"]);
 }
 ?>
<?$APPLICATION->IncludeComponent("bitrix:catalog.section", "random-section", array(
	"IBLOCK_TYPE" => "catalog",
	"IBLOCK_ID" => "2",
	"SECTION_ID" => "",
	"SECTION_CODE" => "",
...
$APPLICATION->SetPageProperty("COMPONENT.SECTION", ob_get_clean()); 
//r($arResult);
?>	
